...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description: {
      en: "Appointment booking for Nova Studio, a hair salon, on Laravel + Inertia + Vue 3 with MySQL. Three surfaces: a public flow that walks a client through service, day and time; a customer area to see and cancel their own bookings; and an admin agenda for the day and the week, plus services, opening hours and booking states. Available slots are computed from the opening hours minus what is already booked and the service duration — they are not a pre-seeded table, so the client only ever sees times that are genuinely free. The interesting part is preventing double booking: validating before inserting leaves a race window where two simultaneous requests both pass, so the booking is created inside a locking transaction, with a uniqueness constraint in the database as the last line of defence. The login screen offers one-click admin and customer demo accounts.",
      es: "Reserva de turnos para Nova Studio, una peluquería, sobre Laravel + Inertia + Vue 3 con MySQL. Tres caras: un flujo público que lleva al cliente por servicio, día y horario; un área de cliente para ver y cancelar sus propias reservas; y una agenda de administración del día y de la semana, más servicios, horarios de atención y estados de las reservas. Los huecos disponibles se calculan a partir de los horarios de atención menos lo ya reservado y la duración del servicio: no son una tabla precargada, así que el cliente solo ve horarios que están realmente libres. Lo interesante es impedir la doble reserva: validar antes de insertar deja una ventana en la que dos pedidos simultáneos pasan los dos, así que la reserva se crea dentro de una transacción con bloqueo, más una restricción de unicidad en la base como última línea de defensa. El login ofrece cuentas de demostración de administrador y de cliente, de un clic.",
    },
    image: "/projects/laravel-vue-reservas.jpg",
    tags: ["Laravel", "PHP", "Vue.js", "Inertia", "MySQL", "Tailwind CSS", "Booking System"],
    liveUrl: "https://laravel-vue-reservas.vercel.app",
